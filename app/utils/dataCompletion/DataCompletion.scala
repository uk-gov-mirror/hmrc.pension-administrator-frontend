...
class DataCompletion {
  ...
  def isCompanyDetailsComplete(ua: UserAnswers): Boolean = {
    ...
    val isAllComplete = isComplete(allAnswers)
    logger.debug(s"isCompanyDetailsComplete: allAnswers $allAnswers, isComplete:$isAllComplete")
    isAllComplete.getOrElse(false)
  }
